...
tomap <- tidalcreeks %>%
  dplyr::inner_join(tidres, by = c('id', 'wbid', 'JEI', 'class')) %>%
  dplyr::mutate(
...
# context indicators for selected creek
cntcrk <- reactive({
  
  req(selcrk)
  browser()
  
  # select data, fill data in record if missing years
  out <- cntdat %>% 
    filter(id %in% selcrk) %>% 
    mutate(year = factor(year, levels = seq(yr - 11, yr))) %>% 
...
cntcrkplo <- reactive({
  ...
  p1 <- ggplot(cntcrk, aes(x = year, y = CHLAC)) + 
    geom_bar(stat = 'identity')
  # plot_ly(cntcrk, x = ~year, y = ~CHLAC, type = 'bar') %>% 
    # layout(
      # yaxis = list(title = 'Chlorophyll (ug/L)'),
      # xaxis = list(title = '', type = 'category', categoryorder = 'array', categoryarray = ~year)
    # )
