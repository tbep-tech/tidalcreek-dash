...
cntdat <- anlz_tdlcrkindic(tidalcreeks, iwrraw, yr = yr) %>% 
  select(id, wbid, JEI, class, year, CHLAC, DO, TN, chla_tn_ratio, tsi, no23_ratio)
...
Tidal creeks or tributaries are essential habitats in the Tampa Bay Estuary and serve as important focal points for understanding watershed inputs that affect water quality. A fundamental goal of the Tampa Bay Estuary Program is to develop effective nutrient management strategies to support the ecological function of tidal tributaries. In partnership with Sarasota Bay NEP, Coastal & Heartland NEP, and local government and agency stakeholders, preliminary methods have been developed for assessing the health of tidal tributaries for fisheries based on exceedances of nitrogen concentrations ([2016 report](https://tbeptech.org/TBEP_TECH_PUBS/2016/TBEP_02_16_SW_FL_Tidal_Creeks_Final_Draft_Report_160121.pdf), [2019 report](https://tbeptech.org/TBEP_TECH_PUBS/2019/TBEP_13_19_2019_Tidal_Creeks_Final_Report.pdf)). These assessments can support tracking of water quality management goals and can help refine restoration and management plans in priority tributaries, including those in need of hydrologic restoration that can support critical nursery habitats for sportfishes.
...
Several years of data included in the IWR database are used to estimate the summary assessment categories. Additional water quality parameters in this database can provide context for interpreting the outcomes, e.g., chlorophyll, nitrogen, dissolved oxygen, etc. These data are summarized for individual creeks in the BAR PLOTS and RELATIVE VALUES tabs.  Each plot shows the annual geometric means of additional water quality measurements for the ten years of data that were used to define the score categories in the SUMMARY ASSESSMENTS. Averages for each year of available data at each wbid/JEI combination are returned for chlorophyll, dissolved oxygen, total nitrogen, total phosphorus, trophic state index, and several other parameters.

The BAR PLOTS are annual averages for the six context indicators at the selected creek. Each year has its own unique color to aid comparisons.  These plots can be used to assess:
